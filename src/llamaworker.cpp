...
LlamaWorker::LlamaWorker(QObject *parent)
    : QObject(parent), m_model(nullptr), m_ctx(nullptr),
...
bool LlamaWorker::isModelLoaded() const { return m_model != nullptr; }
...
void LlamaWorker::stop() { m_stopRequested = true; }
...
void LlamaWorker::generate(const QString &prompt,
  ...
  try {
    ...
    if (n_tokens >= n_ctx) {
      emit error(QString("Prompt too long (%1 tokens) for context size (%2). "
    ...
    llama_sampler_chain_add(sampler,
                            llama_sampler_init_top_p(genParams.topP, 1));
    llama_sampler_chain_add(sampler,
                            llama_sampler_init_temp(genParams.temperature));
